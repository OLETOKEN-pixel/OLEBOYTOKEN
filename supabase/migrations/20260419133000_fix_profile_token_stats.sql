-- Fix profile modal token stats so entry cost is not counted twice.
-- This is read-side only: no wallet balances or historical transactions are changed.
...
BEGIN
  ...
  WITH finalized_user_matches AS (
  ...
  match_token_rows AS (
    ...
    JOIN finalized_user_matches fum ON fum.match_id = t.match_id
  ...
  match_token_profit AS (
    SELECT
      payout_amount,
      payout_amount - CASE
        WHEN lock_amount > 0 THEN GREATEST(lock_amount - refund_amount, 0)
        ELSE GREATEST(fee_amount - refund_amount, 0)
      END AS net_profit
    FROM match_token_rows
  )
  SELECT
    COALESCE(SUM(payout_amount), 0),
    COALESCE(SUM(net_profit), 0),
    COALESCE(MAX(net_profit), 0)
  INTO v_total_earned, v_total_profit, v_best_profit
  FROM match_token_profit;
